...
generate_wsdl <- function(service_name,
                          input_fields,
                          output_fields) {
  wsdl <- xml_new_root("wsdl:definitions")
  xml_set_attr(wsdl,
               "targetNamespace",
               paste0("http://avvale.com/", service_name))
  ...
  schema <- xml_add_child(types,
                          "xsd:schema",
                          targetNamespace = paste0("http://avvale.com/", service_name))
    ...
    for (field in fields) {
      ...
      if (is.null(isChild) || !isChild) {
        ...
        if (!is.null(last_mother_id)) {
          ...
          mother_element <- xml_find_first(sequence,
                                           paste0(".//xsd:element[@name='", mother_name, "']"))
  ...
  xml_add_child(response_message,
                "wsdl:part",
                name = "parameters",
                element = "tns:Z_Operacion_Response")
  ...
  xml_add_child(binding,
                "wsoap12:binding",
                style = "document",
                transport = "http://schemas.xmlsoap.org/soap/http")
  
  operation_bind <- xml_add_child(binding, "wsdl:operation", name = "Z_OperacionRequest")
  input_element <- xml_add_child(operation_bind, "wsdl:input")
  xml_add_child(input_element, "wsoap12:body", use = "literal")
  
  output_element <- xml_add_child(operation_bind, "wsdl:output")
  xml_add_child(output_element, "wsoap12:body", use = "literal")
  ...
  xml_add_child(port,
                "soap:address",
                location = paste0("http://avvale.com/", service_name))
...
transform_childless_to_simple <- function(wsdl) {
  ...
  complex_elements <- xml_find_all(wsdl,
                                   "//xsd:complexType[not(./xsd:sequence/xsd:element)]/..")
  ...
  tags$footer(
    tags$p(
      paste(
        "©",
        format(Sys.Date(), "%Y"),
        "Manuel Almagro (manuel.almagro@avvale.com)",
        "🌱 Avvale"
      ),
      style = "font-size: 12px; text-align: center; margin: 10px 0;"
    ),
...
server <- function(input, output, session) {
  rv <- reactiveValues(
    input_fields = list(list(
      id = 1, name = "Campo1", type = "xsd:string"
    )),
  ...
  observeEvent(input$add_input_field, {
    update_input_fields()
    new_id <- if (length(rv$input_fields) == 0)
      1
    else
      max(sapply(rv$input_fields, `[[`, "id")) + 1
    rv$input_fields <- append(rv$input_fields, list(list(
      id = new_id,
      name = "",
      type = "xsd:string"
    )))
  ...
  observeEvent(input$add_output_field, {
    ...
    new_id <- if (length(rv$output_fields) == 0)
      1
    else
      max(sapply(rv$output_fields, `[[`, "id")) + 1
    rv$output_fields <- append(rv$output_fields, list(list(
      id = new_id,
      name = "",
      type = "xsd:string"
    )))
  ...
  observeEvent(input$branch_input_field, {
    ...
    rv$input_fields <- append(rv$input_fields, list(
      list(
        id = new_id,
        name = "",
        type = "xsd:string",
  ...
  observeEvent(input$branch_output_field, {
    ...
    # Add the new field with isChild and motherId set
    rv$output_fields <- append(rv$output_fields, list(
      list(
        id = new_id,
        name = "",
        type = "xsd:string",
  ...
  output$input_fields_ui <- renderUI({
    header_row <- fluidRow(column(4, h5("Nombre")), column(4, h5("Tipo")), column(4, h5("Opciones")))
    
    input_ui <- lapply(rv$input_fields, function(field) {
      indent_class <- ifelse(field$isChild, "child-field", "")
      fluidRow(class = indent_class,
               column(4, textInput(
                 paste0("input_field_name_", field$id), "", value = field$name
               )),
               column(
                 4,
                 selectInput(
                   paste0("input_field_type_", field$id),
                   "",
                   choices = c("xsd:string", "xsd:int", "xsd:boolean"),
                   selected = field$type
                 )
               ),
               column(4, fluidRow(
                 actionButton(
                   paste0("remove_input_", field$id),
                   "",
                   icon = icon("trash"),
  ...
  output$output_fields_ui <- renderUI({
    ...
    output_ui <- lapply(rv$output_fields, function(field) {
      ...
      fluidRow(class = indent_class,
               column(4, textInput(
                 paste0("output_field_name_", field$id), "", value = field$name
               )),
               column(
                 4,
                 selectInput(
                   paste0("output_field_type_", field$id),
                   "",
                   choices = c("xsd:string", "xsd:int", "xsd:boolean"),
                   selected = field$type
                 )
               ...
               column(4, fluidRow(
                 actionButton(
                   paste0("remove_output_", field$id),
                   "",
                   icon = icon("trash"),
                   style = "margin-top: 25px;"
                 ),
